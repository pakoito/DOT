...
SideBar::SideBar(QHBoxLayout *layout, QWidget *parent)
...
void SideBar::paintEvent(QPaintEvent *event)
{
    ...
    foreach(QAction *action, _actions)
    {
        ...
        {
			p.setOpacity(.5);
			p.fillRect(actionRect, QColor(19, 19, 19, alpha));
			p.setPen(QColor(9, 9, 9, alpha));
            p.drawLine(actionRect.topLeft(), actionRect.topRight());
            p.setOpacity(1);
			if (l != active) //First time we're hitting this
			{
				if (active != -1) //This isn't the first widget we've added
				{
					this->_widgets.at(active)->hide(); //Remove our current widget
				}
				active = l;
				if (this->PresentWidget() == 0)
					break;
			}
        }
		...
		p.setPen(QColor(217, 217, 217, alpha));
        if(action->isChecked())
...
void SideBar::addAction(QAction *action, QWidget* link, const Qt::AlignmentFlag ag)
{
    _actions.push_back(action);
	this->_widgets.push_back(link);
    action->setCheckable(true);
	this->Alignment.push_back(ag);
    update();
    return;
}

QAction *SideBar::addAction(const QString &text, QWidget* link, const Qt::AlignmentFlag ag, const QIcon &icon)
{
